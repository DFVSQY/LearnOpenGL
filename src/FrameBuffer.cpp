 ...
 * 
 * 1. RenderBuffer Object（RBO）
 * 
 * 特点:
 * - 专用性：RBO 是专门为附加到 FBO 设计的图像存储对象。它主要用于存储渲染结果，例如深度缓冲、模板缓冲或多重采样的颜色缓冲。
 * - 不直接采样：RBO 存储的数据不能直接用于纹理采样。换句话说，无法在着色器中直接访问 RBO 中的数据。
 * - 效率：由于其专用性，RBO 可能在某些硬件实现中具有比纹理对象更高的性能，特别是在不需要采样数据时（如只需要存储深度或模板信息）。
 * 
 * 适用场景:
 * - 深度缓冲或模板缓冲：如果你需要在 FBO 中附加深度或模板缓冲，且这些缓冲区不需要在后续的渲染处理中使用，那么 RBO 是理想的选择。
 * - 多重采样（MSAA）：在多重采样抗锯齿中，RBO 常用于存储多重采样的颜色、深度或模板缓冲区。最后再通过 `glBlitFramebuffer` 将结果拷贝到一个纹理中。
 * 
 * 2. Texture Object
 * 
 * 特点:
 * - 通用性：纹理对象可以用于多个用途，包括纹理映射、数据存储和采样。它可以附加到 FBO 以存储渲染结果，并且可以在后续渲染中被着色器采样使用。
 * - 直接采样：纹理对象存储的数据可以直接在着色器中通过纹理采样器（如 `sampler2D`）访问。
 * - 更多格式支持：纹理对象支持多种格式和内部存储模式，使其更加灵活。
 * 
 * 适用场景:
 * - 后处理效果：如果你需要对渲染结果进行后处理（例如模糊、颜色调整等），纹理对象是必须的，因为它允许将渲染结果作为输入纹理进行进一步的处理。
 * - 动态反射或阴影贴图：在环境映射或阴影映射中，渲染结果通常需要存储为纹理并在后续的渲染过程中使用。
 * 
 * 3. 选择和互换
 * 
 * 选择的原则:
 * - 是否需要采样：如果渲染结果需要在后续的渲染中采样使用（如后处理、环境映射），你应该选择纹理对象。如果渲染结果只需要存储（如深度测试），但不需要采样，RBO 是更好的选择。
 * - 性能考虑：在不需要采样的场景下，使用 RBO 可能带来更高的性能，因为它是专门为此类用途设计的。
 * 
 * 是否可以互换:
 * - 不能完全互换：RBO 和纹理对象并不能完全互换，因为它们的设计初衷不同。纹理对象可以在 FBO 和着色器之间共享数据，而 RBO 则不能直接在着色器中采样。
 * - 多重采样处理：在多重采样场景中，你可能需要将 RBO 的内容拷贝到一个纹理对象中，以便后续的处理。因此，在某些特定情况下，可能会用到两者的组合，但不是直接互换。
 * 
 * 4. 实际开发中的建议
 * 
 * - 深度和模板缓冲区：优先使用 RBO，因为它的性能通常更好，且不需要采样。
 * - 颜色缓冲区：如果你只需要存储渲染结果，而不需要后续采样，RBO 是一个好的选择。但如果你需要使用渲染结果进行后处理或在其他渲染步骤中使用，那么应该使用纹理对象。
 * - 后处理管线：通常选择纹理对象，因为渲染的中间结果需要在多个渲染步骤中被访问和修改。
 * 
 * 总结
 * 
 * `RenderBuffer Object` 和 `Texture Object` 在 FBO 中扮演不同的角色，主要区别在于是否需要对存储的数据进行采样。了解这些区别有助于在实际开发中根据具体需求做出合理的选择，以优化性能和功能。
*/
